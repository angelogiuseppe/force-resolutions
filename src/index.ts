...
async function forceResolutions() {
  try {
    ...
    const defaultPackageLockJsonPath = join(
      currentWorkingDirectory,
      "package-lock.json"
    );

    // Construct default package json path
    const defaultPackageJsonPath = join(
      currentWorkingDirectory,
      "package.json"
    );
    ...
    const [packageLockJsonExists, packageJsonExists] = await Promise.all(
      [defaultPackageLockJsonPath, defaultPackageJsonPath].map(
        checkIfFileExists
      )
    );
    ...
    if (packageLockJsonExists && packageJsonExists) {
      ...
      const [packageLockJSON, packageJSON] = await Promise.all(
        [defaultPackageLockJsonPath, defaultPackageJsonPath].map((file) =>
      ...
      if (resolutions) {
        ...
        Object.keys(resolutions).forEach((resolution) => {
          ...
          const keyPaths = findKeyPaths(packageLockJSONContent, resolution);
          ...
          keyPaths.forEach((keyPath) => {
            ...
            // Handle npm 7 package lock json format
              ...
              getModifications(
                keyPath.map((currentKey) =>
                  currentKey === "requires" ? "dependencies" : currentKey
                ),
                {
        ...
        if (changes > 0) {
          ...
          console.log(kleur.green("Finished applying forced resolutions."));
        } else {
          console.log(kleur.green("All resolutions already up-to-date."));
        }
      }
